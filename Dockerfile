...
RUN make -j$(nproc)
RUN install /root/upx/src/upx.out /usr/local/bin/upx

WORKDIR /root/caddy
RUN wget -qO- github.com/caddyserver/caddy/archive/"$CADDY_HASH".tar.gz | tar zx --strip-components=1
RUN cd cmd/caddy && CGO_ENABLED=0 go build && strip --strip-unneeded caddy && upx caddy

FROM alpine
COPY --from=builder /root/caddy/cmd/caddy/caddy /usr/local/bin
